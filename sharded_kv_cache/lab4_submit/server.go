...
func (server *KvServerImpl) GetUsableClient(nodes []string) (proto.KvClient, error, string) {
	for _, node := range nodes {
		...
		// fmt.Printf("got an error when GetUsableClient for node %v. error: %v", node, err1)
	}
	
	return nil, status.Errorf(codes.NotFound, "No available nodes"), ""
}

func (server *KvServerImpl) copyShardData(ctx context.Context, shard_id int) error {
	nodes := server.shardMap.NodesForShard(shard_id) // []string
	// fmt.Printf("node %v is copying shard %v, nodes: %v\n", server.nodeName, shard_id, nodes)

	if len(nodes) == 0 || (len(nodes) == 1 && nodes[0] == server.nodeName) {
		// fmt.Printf("returning error: node %v is copying shard %v, nodes: %v", server.nodeName, shard_id, nodes)
	...
	// fmt.Printf("self node: %v, new node: %v, nodes: %v\n", server.nodeName, new_node, nodes)

	kvClient, err1 := server.clientPool.GetClient(nodes[ind])
	if err1 != nil {
		kvClient, err1, new_node = server.GetUsableClient(nodes)
		if err1 != nil {
			return err1
		}
	}
	
	// fmt.Printf("Node %v is to send GetShardContents for shard %v\n", new_node, shard_id)
	GetShardContentsResponse, err2 := kvClient.GetShardContents(ctx, &proto.GetShardContentsRequest{Shard: int32(shard_id)})

	if err2 != nil {
		reduced_nodes := removeNode(nodes, new_node)
		for len(reduced_nodes) > 0 {
			kvClient, err1, new_node = server.GetUsableClient(reduced_nodes)
			...
			GetShardContentsResponse, err2 = kvClient.GetShardContents(ctx, &proto.GetShardContentsRequest{Shard: int32(shard_id)})
			if err2 == nil {
				server.shardLocks[shard_id].Lock()
				defer server.shardLocks[shard_id].Unlock()
				for _, GetShardValue := range GetShardContentsResponse.GetValues() {
					server.shardData[shard_id].Data[GetShardValue.GetKey()] = ValueWithTtl{
						Value: GetShardValue.GetValue(),
						TimeExpire: time.Now().Add(time.Duration(GetShardValue.GetTtlMsRemaining()) * time.Millisecond),
					}
				}
				return nil
			}
...
func (server *KvServerImpl) handleShardMapUpdate() {
	...
	for _, s := range newShards {
		if !contains(s, server.hostedShards) {
			...
			// fmt.Printf("Adding shard %v to node %v", s, server.nodeName)
			server.copyShardData(context.Background(), s)
	...
	for {
		...
		default:
			...
			for ind, shard := range server.shardData {
				_, exists := server.shardLocks[ind]
				if exists {
...
func MakeKvServer(nodeName string, shardMap *ShardMap, clientPool ClientPool) *KvServerImpl {
	...
	for i:= 1; i <= shardMap.NumShards(); i++ {
		...
		server.copyShardData(context.Background(), i)
